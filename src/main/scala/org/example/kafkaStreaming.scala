  ...
  def streamingCore(sc: StreamingContext): Unit = {
    ...
    stream.foreachRDD(rdd =>
      rdd.foreachPartition { part => {
        //redis connection
        println(hostAndPorts)
        val jedisIns = new JedisCluster(hostAndPorts, redis_timeout)
        //hbase connection
        val config = HBaseConfiguration.create
        config.set("hbase.zookeeper.property.clientPort", "2181")
        config.set("hbase.zookeeper.quorum", "wxh-2019213683-0001")
        val connection = ConnectionFactory.createConnection(config)
        val table = connection.getTable(TableName.valueOf("movie_records"))
        var records: List[Put] = List()
        part.foreach { x => {
          //写入redis
          while (jedisIns.llen("streaming_records") >= 200)
            jedisIns.lpop("streaming_records")
...
//        jedisIns.close()
    ...
    counterUserIdPos.foreachRDD(
      rdd => rdd.foreach { x => {
        ...
        jedisIns.expire("streaming2feature_userId_rating1_" + x._1.toString, 600L)
//        jedisIns.close()
      }
      }
    )
    counterUserIdNeg.foreachRDD(
      rdd => rdd.foreach { x => {
        ...
        jedisIns.expire("streaming2feature_userId_rating0_" + x._1.toString, 600L)
//        jedisIns.close()
      }
      }
    )
    counterMovieIdPos.foreachRDD(
      rdd => rdd.foreach { x => {
        ...
        jedisIns.expire("streaming2feature_movieId_rating1_" + x._1.toString, 600L)
//        jedisIns.close()
      }
      }
    )
    counterMovieIdNeg.foreachRDD(
      rdd => rdd.foreach { x => {
        ...
        jedisIns.expire("streaming2feature_movieId_rating0_" + x._1.toString, 600L)
//        jedisIns.close()
      }
      }
    )
    counterUserId2MovieId.foreachRDD(
      rdd => rdd.foreach { x => {
        ...
        jedisIns.expire(s"streaming2feature_userId_to_genresId_${x._1.toString}_${x._2._1}", 600L)
//        jedisIns.close()
      }
      }
    )
  ...
  def getStream(sc: StreamingContext) = {
    val kafkaParams = Map[String, Object](
      "bootstrap.servers" -> "wxh-2019213683-0001:9092",
